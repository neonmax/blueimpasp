...
public class Handler : IHttpAsyncHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    private static readonly string FILES_PATH = HttpContext.Current.Server.MapPath("");
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        else if (context.Request.HttpMethod.ToUpper() == HttpMethods.POST)
        {
            ...
            for (int FileIndex = 0; FileIndex < context.Request.Files.Count; FileIndex++)
            {
                ...
                string uniqueId = Guid.NewGuid().ToString().Substring(0, 10);
                string uniqueFileName = fileNameWithoutExt + "-" + uniqueId + ext;
                for (int Attempts = 0; Attempts < ATTEMPTS_TO_WRITE; Attempts++)
                {
                    ...
                    try
                    {
                        using (Stream FileStreamWriter = new FileStream(FilesPath + "\\" + uniqueFileName, FileMode.CreateNew, FileAccess.Write))
                        {
                ...
                FileResponseList.Add(CreateFileResponse(context.Request, FilesPath + "\\" + uniqueFileName, File.ContentLength, ErrorMessage));
            }
